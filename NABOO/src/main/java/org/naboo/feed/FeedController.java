...
/*-----------------------------------------------------------------------------------*/
    ...
    public void rimuoviFeed(ActionEvent event) throws IOException {
        ...
        session.createQuery("DELETE org.naboo.feed.Feed WHERE squadra = '" + rim_squadra.getText() + "'").executeUpdate();
    ...
    public void leggiFeed() {
        Session session = HibernateUtil.getSessionAnnotationFactory().openSession();
        Transaction tx = session.beginTransaction();
        Query query = session.createQuery("SELECT org.naboo.feed WHERE ", Feed.class);
        query.executeUpdate();
        tx.commit();
        session.close();
    }
    ...
    public void caricaTabellaFeed(Session session, Transaction tr){
        Query<Feed> query = session.createQuery("FROM org.naboo.feed.Feed", Feed.class);

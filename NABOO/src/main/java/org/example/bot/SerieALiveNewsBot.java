...
class SerieALiveNewsBot extends TelegramLongPollingBot {
    ...
    public String verificaBan(int chat_id) {
        ...
        String query = "FROM org.example.utenti.Utente where chat_id = " + chat_id;
    ...
    public static List<Notizia> ordinaLista(List<Notizia> list){
        List<Notizia> listaOrdinata = new ArrayList<>();
        int j = 0;
        for(int i = list.size(); i != 0; i++){
            listaOrdinata.set(j, list.get(i));
            j++;
        }
    ...
    public void caricaNotizie(List<Notizia> arrayNotizieSquadra, String squadra) {
        ...
        try{
            ...
            Query<Notizia> query = session.createQuery("FROM org.example.notizie.Notizia WHERE squadra = '" + squadra + "' AND timestamp >= '" + dataControllo + "' ORDER BY timestamp", Notizia.class);
            ...
            if (messaggio.hasText()) {
                ...
                if (comando.equals("/start") || flagCommento == true || flagLike == true || flagDislike == true) {
                    ...
                    if (flagFKUtenteCommento == true) {
                        ...
                        try {
                            i = session.createQuery("FROM org.example.notizie.Notizia WHERE notizie_id = " + update.getMessage().getText(), Notizia.class).list();
                        }catch (Exception e){
                            System.out.println(e);
                        }
                        tx.commit();

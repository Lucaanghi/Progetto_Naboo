package org.example.pannelloFx;
...
import org.example.notizie.NewsCollector;
...
/*-----------------------------------------------------------------------------------*/
    ...
    public void validaLogin(ActionEvent event) {
        ...
        Query<Admin> query = session.createQuery("FROM org.example.admin.Admin WHERE email = '" + email.getText() + "' AND password='" + password.getText() + "'", Admin.class);
        List<Admin> boss = query.list();
        try {
            if (boss.size() == 1) {
                messaggioLogin.setText("Login effettuato con successo");
                session.createQuery(" UPDATE org.example.admin.Admin SET login = true WHERE email = '" + email.getText() + "'").executeUpdate();

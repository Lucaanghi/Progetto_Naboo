...
/*-----------------------------------------------------------------------------------*/
    ...
    public void aggiornaDatiAdmin(){
        ...
        if(email.isBlank()  && password.isBlank()){
            ...
            session.createQuery(" UPDATE org.example.admin.Admin SET password = '" + password_admin.getText() + "' WHERE login = true").executeUpdate();
            ...
            session.createQuery(" UPDATE org.example.admin.Admin SET email = '" + email_admin.getText() + "' WHERE login = true").executeUpdate();
    ...
    public void logoutAdmin(ActionEvent event){
        Session session = HibernateUtil.getSessionAnnotationFactory().openSession();
        Transaction tx = session.beginTransaction();
        session.createQuery(" UPDATE org.example.admin.Admin SET login = false WHERE login = true").executeUpdate();
        tx.commit();
        session.close();
        System. exit(0);
    }
    ...
    public String setPromptTextEmail(Session session, Transaction tx){
        Query queryemail = session.createQuery("SELECT email FROM org.example.admin.Admin WHERE login = true");
    ...
    public String setPromptTextPassword(Session session, Transaction tx){
        Query querypassword = session.createQuery("SELECT password FROM org.example.admin.Admin WHERE login = true");
